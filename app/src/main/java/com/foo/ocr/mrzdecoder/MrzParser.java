...
import com.foo.ocr.mrzdecoder.types.MrzDate;
...
//    private static Logger log = LoggerFactory.getLogger(MrzParser.class);
    ...
    public MrzDate parseDate(MrzRange range) {
        ...
        try {
            ...
            year = -1;
//            Log.d("Failed to parse MRZ date year " + rawValue(range) + ": " + ex, mrz +  r);
        }
        if (year < 0 || year > 99) {
//            log.d("Invalid year value " + year + ": must be 0..99");
        ...
        try {
            ...
            month = -1;
//            log.d("Failed to parse MRZ date month " + rawValue(range) + ": " + ex, mrz, r);
        }
        if (month < 1 || month > 12) {
//            log.d("Invalid month value " + month + ": must be 1..12");
        }
...
//            log.d("Failed to parse MRZ date month " + rawValue(range) + ": " + ex, mrz, r);
        ...
        if (day < 1 || day > 31) {
//            log.d("Invalid day value " + day + ": must be 1..31");
        }
